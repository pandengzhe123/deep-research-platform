...
> 最后更新：2026年6月8日 | 代码 ~3,500 行 + 文档 ~3,200 行
...
| JWT 用户认证 | ✅ | 注册/登录/JWT签发/Principal提取/多租户隔离 |
| 会话按用户过滤 | ✅ | listSessions 走 ReactiveSecurityContextHolder |
| KB 多租户隔离 | ✅ | list_docs/delete_doc 按 user_id 过滤 |
| 费用估算 | | 后续 |
...
1. **Docker Compose 一键部署** —— `docker compose up` 一行启动
2. **前端框架升级** —— Vue/React
3. **Spring Security 强制认证** —— 等前端完全适配后开启
4. **SSE 流式** —— 已知限制，暂不开发
