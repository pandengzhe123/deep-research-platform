...
> 约 3,200 行代码（Python ~1,800 + Java ~700 + Vue ~650）、四个服务。完整的 Deep Research Agent 平台。
...
Python Agent: 95% — Level 1-4 + 错误恢复 + 双搜索 + RAG + OOM/Token 保护
Java 网关:   85% — JWT + 会话持久化 + 并发控制 + 30 分钟超时
前端:        85% — Vue 3 聊天界面 + 会话管理 + KB 面板 + 报告导出
RAG:         95% — Chroma + embedding + 多租户 + Level 3/4 贯通
测试:        60% — 单元 14 例 + 回归 6 例
部署:        80% — Docker Compose 4 服务 + 全链路超时对齐
文档:        95% — 15 个文件 ~5,000 行
```
