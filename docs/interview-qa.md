...
3. **长报告的 token 超限风险**。已实现三层保护：单个搜索结果 30 万字符截断 + 只保留最近 3 轮结果 + 压缩前 50 万字符截断。消息历史超 50 万字符时截断旧消息但始终保留原始问题（messages[0]）。全部搜索失败时直接返回兜底提示不走 LLM。但对于极端超长对话历史（用户多轮追问积累），context 无截断仍是已知限制。
...
"已经有两套测试：
1. **`test_units.py`（14 个用例）**——纯函数级测试，1 秒跑完。覆盖 LLM 响应解析、搜索结果格式化、配置加载等。
2. **`test_quality.py`**——3 题 × 2 Level 端到端测试，用 4 条正则规则检查报告质量（标题、引用、来源章节、长度），附带 LLM-as-judge 自动评分（仅参考，不计通过率）。

后续改进方向：
- **回归测试**——预录 Tavily/DeepSeek API 的 mock 数据，用 `unittest.mock` 替换真实 API。每次改 Agent 逻辑后跑一遍同样输入对比输出。
- **CI 集成**——当前只能手动跑，后续接 GitHub Actions。"

> **考官想听**：有测试意识，已有基础覆盖，知道下一步怎么加强。
...
"最大的技术遗憾是 SSE 流式推送——最初 `run_agent_with_sse()` 在 server.py 里重新实现了一遍 Agent 逻辑（~460 行），和 agent.py 的类完全重复。Agent 逻辑分散在两个文件，改一处漏一处。

后来我做了一次关键重构：给每个 Agent 类加 `on_progress` 回调参数，内部关键步骤调 `self.emit(event)` 通知外部。server.py 只负责注册回调、把事件放进 Queue、从 Queue 读取并 yield SSE。重构后 server.py 从 ~460 行缩到 ~280 行，Agent 逻辑 100% 收敛到 agent.py。

这让我深刻理解了**关注点分离**——进度推送是展示层的事，不应该侵入 Agent 核心逻辑。回调模式是观察者模式的最简实现。"

> **考官想听**：你有自我反思能力，发现问题后主动重构，能用设计模式解释为什么。
