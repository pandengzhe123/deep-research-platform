...
> 共 24 问，分八类：架构设计、效果边界、工程实践、学习动机、性能成本、异步并发、工程细节、深度追问。每题附考官视角。
...
"目前是直接报错返回给用户。这不是最优解，但是诚实的当前状态。

要加的三层保护：
1. **搜索降级**——Tavily 挂了自动切换 DuckDuckGo（免费，不需要 API Key）
2. **模型降级**——DeepSeek 挂了自动切换 OpenAI（如果有 Key）
3. **Java 网关熔断**——用 Resilience4j 的 CircuitBreaker，连续失败 5 次就熔断，30 秒后重试

这三层是按优先级排的——当前项目已在 PROGRESS.md 里标注为后续迭代。"

> **考官想听**：你知道当前状态的限制，且有明确的改进计划。
...
技术上：彻底理解了 Function Calling 不是框架特性，是 API 能力。理解了 Agent 循环的本质是 while + tool_calls + messages 追加——这 20 行代码比 LangChain 任何 Tutorial 都更能说清楚什么是 Agent。

架构上：学会了'不改推理层只加网关层'的设计模式。现在给我一个 ML 模型，我能在 Java 侧建一个工程化的壳来服务它——这不只是这个项目的经验，是可以迁移到更多场景的架构肌肉记忆。

最后一个是面试官可能更关心的——**我学会了判断什么时候需要框架、什么时候不需要。** LangGraph 有价值，但如果我连它底层做什么都不懂，我用它也只是在调 API。写过一遍底层之后，我再去用 LangGraph 能写出更好的代码——因为我理解 bind_tools 背后在干什么、state 每次是怎么合并的。"
