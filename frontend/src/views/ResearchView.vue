...
function saveChatToCache(sessionId, messages) {
  localStorage.setItem('chat_' + sessionId, JSON.stringify(messages))
  // 清理：localStorage 超过 3MB 时删除最旧的会话
  let total = 0
  const keys = []
  for (let i = 0; i < localStorage.length; i++) {
    const k = localStorage.key(i)
    if (k.startsWith('chat_')) {
      total += localStorage.getItem(k).length * 2  // UTF-16 → bytes
      keys.push(k)
    }
  }
  if (total > 3 * 1024 * 1024 && keys.length > 5) {
    keys.sort((a, b) => (a < b ? -1 : 1))
    for (const k of keys) {
      localStorage.removeItem(k)
      total -= localStorage.getItem(k)?.length * 2 || 0
  ...
  function handleEvent(eventName, eventData) {
    ...
    if (eventName === 'session') {
      ...
      try {
        ...
        if (mySessionId) {
          saveChatToCache(mySessionId, myMessages)
          loadSessions()
        }
        // 如果用户正在看这个会话，刷新 UI
      ...
      if (mySessionId) saveChatToCache(mySessionId, myMessages)
      if (isViewing()) { messages.value = myMessages; scrollDown() }
    }
  }
