...
class FastLevel1Agent:
    ...
    def __init__(self, on_progress=None, kb_enabled=False, user_id: str = "default"):
        self.llm = LLMClient()
    ...
    async def _kb_search(self, query: str) -> str:
        ...
        return await asyncio.to_thread(self.kb.search, query, user_id=self.user_id)
...
class Level2Agent:
    ...
    def __init__(self, on_progress=None, kb_enabled: bool = False, user_id: str = "default", llm=None):
        self.llm = llm or LLMClient()  # 共享客户端复用连接，减少 TCP 连接数
                ...
                for tc in msg.tool_calls:
                    ...
                    try:
                        if name == "search":
                            ...
                            if self.kb_enabled:
                                result = await asyncio.to_thread(self.kb.search, query, user_id=self.user_id)
                            else:
                                result = "知识库未启用。"
                            if len(result) > MAX_RESULTS_CHARS:
                                result = result[:MAX_RESULTS_CHARS] + "\n\n（结果过长已截断）"
...
class Level3Agent:
    ...
    def __init__(self, on_progress=None, kb_enabled: bool = False, user_id: str = "anonymous"):
        self.llm = LLMClient()
        self.emit = on_progress or (lambda e: None)
        self.kb_enabled = kb_enabled
        self.user_id = user_id
    ...
    async def run(self, question: str) -> str:
        ...
        async def safe_run(topic, idx):
            try:
                agent = Level2Agent(on_progress=self.emit, kb_enabled=self.kb_enabled, user_id=self.user_id, llm=self.llm)
