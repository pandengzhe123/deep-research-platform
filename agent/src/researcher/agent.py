...
class Level3Agent:
    ...
    def __init__(self, on_progress=None, kb_enabled: bool = False, user_id: str = "anonymous", rag_doc_ids: list[str] = None):
        self.llm = LLMClient()
        self.emit = on_progress or (lambda e: None)
        self.kb_enabled = kb_enabled
        self.user_id = user_id
    ...
    async def run(self, question: str) -> str:
        ...
        async def safe_run(topic, idx):
            try:
                agent = Level2Agent(on_progress=self.emit, kb_enabled=self.kb_enabled, user_id=self.user_id, llm=self.llm, rag_doc_ids=self.rag_doc_ids)
