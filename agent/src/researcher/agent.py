...
class FastLevel1Agent:
    ...
    async def _kb_search(self, query: str) -> str:
        """异步知识库检索（包装成 awaitable）。"""
        return self.kb.search(query, user_id=self.user_id)
...
class Level2Agent:
    ...
    def __init__(self, on_progress=None, kb_enabled: bool = False, user_id: str = "default"):
        self.llm = LLMClient()
            ...
            try:
                ...
                for tc in msg.tool_calls:
                    ...
                    if name == "search":
                        ...
                        result = self.kb.search(query, user_id=self.user_id) if self.kb_enabled else "知识库未启用。"
                        messages.append({
                            "role": "tool",
                            "tool_call_id": tc.id,
                            "content": result,
...
class Level3Agent:
    ...
    async def run(self, question: str) -> str:
        ...
        async def safe_run(topic, idx):
            try:
                agent = Level2Agent(on_progress=self.emit)
        ...
        # Step 3: 汇总
        print(f"\n[3/3] 汇总所有子报告...")
        self.emit({"step": "reporting", "message": f"所有研究员完成，正在汇总 {len(reports)} 份子报告..."})
        merged = "\n\n---\n\n".join(
            f"## 子课题{i+1}: {t}\n\n{r}"
...
class Level4Agent:
    ...
    async def run(self, question: str) -> str:
        ...
        for round_num in range(1, self.max_rounds + 1):
            ...
            # 并行派遣研究员
            self.emit({"step": "searching", "message": f"派遣 {len(conduct_calls)} 个研究员并行工作", "round": round_num})
